...
public class SubstepsLaunchConfigurationDelegate extends AbstractJavaLaunchConfigurationDelegate {
	 ...
	 */
	@Override
	public synchronized void launch(final ILaunchConfiguration configuration, String mode, final ILaunch launch, IProgressMonitor monitor) throws CoreException {
		if (monitor == null) {
		...
		try {
			if (mode.equals(SubstepsLaunchConfigurationConstants.MODE_RUN_QUIETLY_MODE)) {
			...
			keepAlive = mode.equals(ILaunchManager.DEBUG_MODE) && configuration.getAttribute(SubstepsLaunchConfigurationConstants.ATTR_KEEPRUNNING, false);
			port = evaluatePort();
	...
	private int evaluatePort() throws CoreException {
		...
		if (port == -1) {
			abort(SubstepsFeatureMessages.SubstepsLaunchConfigurationDelegate_error_no_socket, null, IJavaLaunchConfigurationConstants.ERR_NO_SOCKET_AVAILABLE);
		}
		return port;
	}

	/**
	 * Performs a check on the launch configuration's attributes. If an attribute contains an invalid value, a {@link CoreException} with the error is thrown.
	 * 
	 * @param configuration
	...
	/**
	 * Evaluates all test elements selected by the given launch configuration. The elements are of type {@link IType} or {@link IMethod}. At the moment it is
	 * only possible to run a single method or a set of types, but not mixed or more than one method at a time.
	 * 
	 * @param configuration
	...
	protected IMember[] evaluateTests(final ILaunchConfiguration configuration, final IProgressMonitor monitor) throws CoreException {
		...
		final String testMethodName = configuration.getAttribute(SubstepsLaunchConfigurationConstants.ATTR_TEST_METHOD_NAME, ""); //$NON-NLS-1$
	 ...
	 * @param vmArguments
	 *            a {@link List} of {@link String} representing the resulting VM arguments
	 * @param programArguments
	 *            a {@link List} of {@link String} representing the resulting program arguments
	 * @exception CoreException
	 *                if unable to collect the execution arguments
	 */
	protected void collectExecutionArguments(final ILaunchConfiguration configuration, final List<String> vmArguments, final List<String> programArguments)
	        throws CoreException {
		...
		final String testFailureNames = configuration.getAttribute(SubstepsLaunchConfigurationConstants.ATTR_FAILURES_NAMES, ""); //$NON-NLS-1$
		...
		if (testElements.length == 1) {
			if (testElements[0] instanceof IMethod) {
				...
				final IType type = (IType) testElements[0];
			...
			try {
				...
				for (int i = 0; i < testElements.length; i++) {
					if (testElements[i] instanceof IType) {
						final IType type = (IType) testElements[i];
	...
	public String[] getClasspath(final ILaunchConfiguration configuration) throws CoreException {
		...
		final List<String> junitEntries = new ClasspathLocalizer(Platform.inDevelopmentMode()).localizeClasspath(FeatureRunnerPlugin.PLUGIN_ID);
	 ...
	 * 
	 * public ClasspathLocalizer(final boolean inDevelopmentMode) { fInDevelopmentMode = inDevelopmentMode; }
	 * 
	 * public List localizeClasspath(final ITestKind kind) { final JUnitRuntimeClasspathEntry[] entries= kind.getClasspathEntries(); final List junitEntries=
	 * new ArrayList();
	 * 
	 * for (int i= 0; i < entries.length; i++) { try { addEntry(junitEntries, entries[i]); } catch (final IOException e) { Assert.isTrue(false,
	 * entries[i].getPluginId() + " is available (required JAR)"); //$NON-NLS-1$ } } return junitEntries; }
	 * 
	 * private void addEntry(final List junitEntries, final JUnitRuntimeClasspathEntry entry) throws IOException, MalformedURLException { final String
	 * entryString= entryString(entry); if (entryString != null) junitEntries.add(entryString); }
	 * 
	 * private String entryString(final JUnitRuntimeClasspathEntry entry) throws IOException, MalformedURLException { if (inDevelopmentMode()) { try { return
	 * localURL(entry.developmentModeEntry()); } catch (final IOException e3) { // fall through and try default } } return localURL(entry); }
	 * 
	 * private boolean inDevelopmentMode() { return fInDevelopmentMode; }
	 * 
	 * private String localURL(final JUnitRuntimeClasspathEntry jar) throws IOException, MalformedURLException { final Bundle bundle=
	 * JUnitCorePlugin.getDefault().getBundle(jar.getPluginId()); URL url; if (jar.getPluginRelativePath() == null) url= bundle.getEntry("/"); //$NON-NLS-1$
	 * else url= bundle.getEntry(jar.getPluginRelativePath()); if (url == null) throw new IOException(); return FileLocator.toFileURL(url).getFile(); } }
	 */

	private final IJavaElement getTestTarget(final ILaunchConfiguration configuration, final IJavaProject javaProject) throws CoreException {
		final String containerHandle = configuration.getAttribute(SubstepsLaunchConfigurationConstants.ATTR_TEST_CONTAINER, ""); //$NON-NLS-1$
		if (containerHandle.length() != 0) {
			final IJavaElement element = JavaCore.create(containerHandle);
			if (element == null || !element.exists()) {
				abort(SubstepsFeatureMessages.SubstepsLaunchConfigurationDelegate_error_input_element_deosn_not_exist, null,
				        IJavaLaunchConfigurationConstants.ERR_UNSPECIFIED_MAIN_TYPE);
			}
			return element;
		}
