...
public class ContentFormattingSteps extends AbstractEditorSteps {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ContentFormattingSteps.class);
	...
	public void ensureNewEditor(final String featureFileName,
			final String projectName) {

		final FeatureFileEditorController controller = createController(FeatureFileEditorController.class);

		if (controller.doesFeatureFileExistForProject(featureFileName,
				projectName)) {
		...
		new ProjectSWTController().selectFileToEdit(projectName,
				featureFileName);
	}
	...
	public void assertEditorContentsMatch(final String fileName) {
		...
		final String textFromEditor = removeTrailingNewlines(controller
				.currentEditorContents());

		LOG.debug("About to check that text from editor [" + textFromEditor
				+ "] matches expected text from file [" + textFromFile
				+ "] in file [" + fileName + "]");

		assertEquals("editor contents are not the same as the contents in "
				+ fileName, textFromFile, textFromEditor);
	}
	...
	private String textFromFile(final String fileName) {
		try {
			final InputStream stream = getClass().getResourceAsStream(
					"/featuresnippets/" + fileName);
			if (stream == null) {
				throw new IOException("Could not get contents from file "
						+ fileName);
			}
