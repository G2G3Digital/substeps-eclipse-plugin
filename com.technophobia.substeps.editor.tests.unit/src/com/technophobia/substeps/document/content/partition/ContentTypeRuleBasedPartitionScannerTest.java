...
public class ContentTypeRuleBasedPartitionScannerTest {
	...
	public void initScanner() {
		this.partitionScanner = new ContentTypeRuleBasedPartitionScanner(
	...
	public void canPartitionComments() {
		...
		final ITypedRegion[] result = partitioner.computePartitioning(0,
				text.length());
		assertEquals(4, result.length);

		checkType(result[0], FeatureContentTypeDefinition.COMMENT.id(),
				"#This is comment line 1\n");
		checkType(result[3], FeatureContentTypeDefinition.COMMENT.id(),
				"#This is comment line 2");
	}
	...
	public void canPartitionBackground() {
		...
		checkType(result[1], FeatureContentTypeDefinition.BACKGROUND.id(),
				"Background:\n");
	}
	...
	public void canPartitionScenario() {
		...
		checkType(result[5], FeatureContentTypeDefinition.SCENARIO.id(),
				"Scenario:A scenario");
	}
	...
	public void canPartitionScenarioOutline() {
		...
		checkType(result[5],
				FeatureContentTypeDefinition.SCENARIO_OUTLINE.id(),
				"Scenario Outline:A scenario");
	}
	...
	public void canPartitionExample() {
		...
		checkType(result[6], FeatureContentTypeDefinition.EXAMPLE.id(),
				"Examples:\n");
	}
	...
	public void canPartitionExampleRow() {
		...
		checkType(result[8], FeatureContentTypeDefinition.EXAMPLE_ROW.id(),
				"|example1|example2|");
	}
	...
	public void canPartitionGiven() {
		...
		checkType(result[2], FeatureContentTypeDefinition.GIVEN.id(),
				"Given Something\n");
	}
	...
	public void canPartitionWhen() {
		...
		checkType(result[3], FeatureContentTypeDefinition.WHEN.id(),
				"When something else\n");
	}
	...
	public void canPartitionThen() {
		...
		checkType(result[4], FeatureContentTypeDefinition.THEN.id(),
				"Then a result");
	}
	...
	public void canPartitionAnd() {
		...
		checkType(result[4], FeatureContentTypeDefinition.AND.id(),
				"And another thing\n");
	}

	private void checkType(final ITypedRegion typedRegion, final String type,
			final String text) {
		assertThat(typedRegion.getType(), is(type));
		assertThat(typedRegion.getLength(), is(text.length()));
	}

	private IDocumentPartitioner createPartitionerForDocumentWithText(
			final String text) {
